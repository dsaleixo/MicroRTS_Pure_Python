...
class TwoLevelsearch:
    ...
    def __init__(self) :
        self.eval=EvaluatorSP(3,FeatureFactory1()); # 1 = BestREsponde inf = FictionPlay, recomento usar 2 ou 3, mas para ser rapido mellhor deixar 1
    ...
    def run(self, gs : GameState, max :int) ->None:
        while(True):
            ...
            print("select: "+seed[0].toString())
            oracle = Feature1(BehavioralFeature(4,7,7,7,0,1,10))
            seed = (oracle, self.l1.getScript(seed[0]))
            print("xxxxxxxxxxxxxxx")
            print(self.l1.toString())
            print("select: "+seed[0].toString())
            print("initial script "+seed[1].translate())# usando a memoria do NS iniciamos a busca com um script proximo
            c0 :Node = self.l2.run(gs, max, seed[1],seed[0], self.eval,self.l1); #realiza a busca para o vetor de caracterista
            self.eval.update(gs, max, c0)#tentamos atualizar o vetor de inimigos do SelfPlay usando o script encontrado
            print(self.l1.toString())
            #self.l1.toString2()
